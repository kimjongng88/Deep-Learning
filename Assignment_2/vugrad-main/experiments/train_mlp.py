...
if args.data == 'synth':
    ...
    raise Exception("asdf")#(f'Dataset {args.data} not recognized.')
...
for epoch in range(args.epochs):
    ...
    loss_aa[epoch + 1] = round(cl/n,4)
    ac_aa[epoch + 1] = round(acc,4)
    #acc.append({acc:.4})

    with open("relu_acc.txt", 'w') as f: 
        for key, value in ac_aa.items(): 
            f.write('%s:%s\n' % (key, value))

    
    with open("relu_loss.txt", 'w') as f: 
        for key, value in loss_aa.items(): 
            f.write('%s:%s\n' % (key, value))



#python vugrad-main/experiments/train_mlp.py
#python vugrad-main/experiments/train_mlp.py -D mnist -l 0.0001
